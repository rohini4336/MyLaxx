...
1. Running the mylaxx `start` or `init` commands the very first time using the `--password` flag (followed by your password) will encrypt the database with that password. Note:
the password will be visible in your terminal if you launch from the terminal.
2. Running the mylaxx `encryptdatabase` at any time will enable you to encrypt the database. (Unlike the previous option, the terminal input will be obfuscated.)
...
If you are running the mylaxx daemon on a remote machine you MUST enable API authentication otherwise anyone will be able to log into your
...
The settings to enable authentication are found in the config file located in the `mylaxx` data directory. To enable authentication first set the
...
You can have mylaxx hash and save the username and password for you by running the `setapicreds` command.
...
The instructions to set up SSL can be found in a separate [doc](https://github.com/MyLaxx/ssl.md). 
...
```
Or pass them in at start up: `mylaxx start -a 69.89.31.226`
